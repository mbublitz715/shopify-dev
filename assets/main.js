...
//         }, 1500);
  ...
  function showKardiaCareOffer() {
    ...
    $('#kardiacare-upsell-add-yearly').on('click', function() {
      var sellingPlan = '4634509415' // yearly selling plan
      kardiaCareCartAdd(kardiacare_id, frequency, unit, sellingPlan);
    });
    $('#kardiacare-upsell-add-monthly').on('click', function() {
      var sellingPlan = '4634574951' // monthly selling plan
      kardiaCareCartAdd(kardiacare_id, frequency, sellingPlan);
    });
  ...
  $('.plan-selector label').on('click', function() {
    ...
    var price = $(this).siblings('.radio').data().price;
...
function addToCartKardiaCareRecurring(event, form) {
  ...
  var sellingPlan = $('input[name=kardiaCareMembershipLength]:checked').data().sellingplan;
  var unit = "Months";
  console.log({ kardiacare_id, frequency, unit, sellingPlan });
  kardiaCareCartAdd(kardiacare_id, frequency, unit, sellingPlan);
}
...
function addToCartKardiaCareDeviceBundle(event, form, device) {
  ...
  event.preventDefault();
  var kardiacare_id = $('input[name=kardiaCareMembershipLength]:checked', '#kardiaCareBundleAddToCartForm').val();
  var kardiacare_selling_plan = $('input[name=kardiaCareMembershipLength]:checked', '#kardiaCareBundleAddToCartForm').data().sellingplan;
  var frequency = $('input[name=kardiaCareMembershipLength]:checked').data().frequency.toString();
  var unit = "Months";
//   LAUREN TO EDIT CARD ID FOR PROD
  if (device === 'kardiamobile') {
//     device_id = "20036084615";
    device_id = "57806915659";

    
  } else if (device === 'kardiamobile6l') {
//     device_id = "28007532101697";
    device_id = "20642078130279";
     
  } else {
    device_id = "39379428868199";
  }
...
//   if (kardiacare_id !== 'no_mem' && device == 'kardiamobile') {
  if (kardiacare_id != 'no_mem') {
    items.push({
      'quantity': 1,
      'id': kardiacare_id,
      'selling_plan': kardiacare_selling_plan,
      "properties": {
        "shipping_interval_frequency": frequency,
        "shipping_interval_unit_type": unit
      }
...
function kardiaCareCartAdd(kardiacare_id, frequency, unit, sellingPlan) {
  $.ajax({
    type: 'POST',
    url: '/cart/add.js',
    data: {
...
function kardiaCareHigherTierCartAdd() {
  const higherTierVariantId = 39920483958887;
  const sellingPlan = 4634443879;
  ...
  $.ajax({
    ...
    data: {
      'quantity': 1,
      'id': higherTierVariantId,
      "selling_plan": sellingPlan,
      "properties": {
        "shipping_interval_frequency": 'Months',
        "shipping_interval_unit_type": '12'
      }
    },
    dataType: 'json',
    success: function() {
...
function kcpAddToCartWorkaround(sellingPlan) {
  ...
  async function addProductsSequentially() {
    console.log("Sequentially adding products");
